...
const creerArrierePlan = ({ app, Tray, Menu, icone, enPaquet, montrer }) => {
  ...
  let quitter = false;
  let zone = null;
  /** Le dernier choix écrit au registre : null tant qu'on n'y a rien écrit. */
  let inscrit = null;
  ...
  const appliquer = (preferences) => {
    ...
    // — et seulement quand le choix change : les réglages sont réannoncés à
    // chaque enregistrement, quel qu'il soit
    const demarrage = reglage.demarrage === true;
    if (enPaquet && demarrage !== inscrit) {
      app.setLoginItemSettings(reglageDemarrage(demarrage));
      inscrit = demarrage;
    }
  };
  ...
  return { appliquer, surFermeture, demarrerCache, autoriserDepart };
};
