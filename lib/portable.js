...
const IMPORT_TASK_FIELDS = [
  ...
  'reminder',
  'myDay',
  // sans elle, restaurer une sauvegarde viderait le bilan des jours passés
  'completedAt',
];
...
const validateImport = (payload) => {
  ...
  const categoryNames = categories.filter((c, index) => validCategoryRow[index]).map((c) => c.name);
