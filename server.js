...
const { HEX_COLOR } = require('./lib/portable');
const { asString } = require('./lib/entrees');
const { creerRoutesDonnees } = require('./lib/donnees-routes');
...
async function sauvegardeAutomatique() {
  try {
    const { ecrit } = await sauvegarderSiBesoin({
      ...
      collecter: donnees.collectExport,
    });
...
const donnees = creerRoutesDonnees({
  Task,
  Category,
  dossierSauvegardes: () => dossierSauvegardes(),
  echouer: fail,
});
app.use(donnees.routeur);
